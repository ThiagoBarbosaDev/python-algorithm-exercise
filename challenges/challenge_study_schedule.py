def validate_student_presence(student_presence):
    if student_presence[0] is None or student_presence[1] is None or type(student_presence[0]) is str or type(student_presence[1]) is str:
        raise ValueError
...
def is_target_time(student_presence, target_time):
    ...
    most_people = 0
    for student_hour in range(
        student_presence[0], (student_presence[1] + 1)
    ):
